...
@Component({
  selector: 'app-deliberations-jury-page',
  templateUrl: './deliberations-jury-page.component.html',
  styleUrls: ['./deliberations-jury-page.component.scss']
})
export class DeliberationsJuryPageComponent extends BaseComponentClass implements OnInit {
  activeTab: 'deliberations' | 'jury' = 'deliberations';
  ...
  constructor(
    ...
    private juryService: JuryMembreService,
  ...
  setTab(tab: 'deliberations' | 'jury'): void {
    this.activeTab = tab;
  }
  ...
  onItemAction(event: { item: any; action: string }): void {
    if (event.action === 'details') return;
    ...
    this.deliberationServiceAction(event.action, event.item.id).subscribe({
      next: () => this.chargerDeliberations(),
      error: () => this.chargerDeliberations()
    });
  }
  ...
  chargerJury(): void {
    this.juryLoading = true;
    this.juryService.getAll().subscribe({
      next: (res) => { this.juryItems = res; this.juryLoading = false; },
      error: () => { this.juryLoading = false; }
    });
  }
  ...
  supprimerJury(id: number): void {
    if (!confirm('Confirmer la suppression ?')) return;
    this.juryService.delete(id).subscribe(() => {
      this.juryItems = this.juryItems.filter(i => i.id !== id);
    });
  }
