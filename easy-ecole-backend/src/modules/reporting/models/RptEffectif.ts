import { Model, InferAttributes, InferCreationAttributes, CreationOptional, DataTypes } from "sequelize";
import { DatabaseConnection } from "../../../core/helpers/DatabaseConnection";
import { MODULE_MODEL_PREFIX, MODULE_TABLE_PREFIX } from "../ReportingModule";

export class RptEffectif extends Model<InferAttributes<RptEffectif>, InferCreationAttributes<RptEffectif>> {
  declare id: CreationOptional<string>
  declare classeId: number | null
  declare periode: string | null
  declare nbInscrits: CreationOptional<number>
  declare nbActifs: CreationOptional<number>
  declare nbHommes: CreationOptional<number>
  declare nbFemmes: CreationOptional<number>
  declare readonly createdAt: CreationOptional<Date>
  declare readonly updatedAt: CreationOptional<Date>
}

RptEffectif.init({
  id: { type: DataTypes.INTEGER.UNSIGNED, autoIncrement: true, primaryKey: true },
  classeId: { type: DataTypes.INTEGER.UNSIGNED, allowNull: true },
  periode: { type: new DataTypes.STRING, allowNull: true },
  nbInscrits: { type: DataTypes.INTEGER, defaultValue: 0 },
  nbActifs: { type: DataTypes.INTEGER, defaultValue: 0 },
  nbHommes: { type: DataTypes.INTEGER, defaultValue: 0 },
  nbFemmes: { type: DataTypes.INTEGER, defaultValue: 0 },
  createdAt: DataTypes.DATE,
  updatedAt: DataTypes.DATE,
}, {
  sequelize: DatabaseConnection.getInstance().sequelize,
  modelName: MODULE_MODEL_PREFIX + 'Effectif',
  tableName: MODULE_TABLE_PREFIX + 'effectifs',
  timestamps: true
})
