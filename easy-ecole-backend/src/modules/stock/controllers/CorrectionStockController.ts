...
export default class CorrectionStockController {
    constructor() { }

    static async getAll(req: Request, res: Response): Promise<Response> {
        try {
            const items = await CorrectionStock.findAll();
    ...
    static async get(req: Request, res: Response): Promise<Response> {
        try {
            const item = await CorrectionStock.findOne({ where: { id: req.params.id } });
            if (item == null)
    ...
    static async getByArticle(req: Request, res: Response): Promise<Response> {
        try {
            const items = await CorrectionStock.findAll({ where: { articleId: req.params.articleId } });
            return res.status(200).send(items);
        } catch (error) {
            return res.status(500).json({ success: false, error: error });
        }
    ...
    static async create(req: Request, res: Response): Promise<Response | null> {
        ...
        try {
            const item = await CorrectionStock.create({ ...req.body });
            await Article.update({ stockActuel: item.quantiteApres }, { where: { id: item.articleId } });
            return res.status(200).send(item);
        } catch (error: any) {
            return res.status(500).json({ success: false, error: error });
        }
