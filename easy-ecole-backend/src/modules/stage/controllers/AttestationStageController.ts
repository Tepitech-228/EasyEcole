...
import { AttestationStage } from "../models/AttestationStage";

export default class AttestationStageController {
    constructor() { }

    static async getAll(req: Request, res: Response): Promise<Response> {
        try { const items = await AttestationStage.findAll(); return res.status(200).send(items); }
        catch (error) { return res.status(500).json({ success: false, error: error }); }
    }

    static async get(req: Request, res: Response): Promise<Response> {
        try {
            const item = await AttestationStage.findByPk(req.params.id);
    ...
    static async create(req: Request, res: Response): Promise<Response | null> {
        ...
        try {
            const item = await AttestationStage.create({ ...req.body });
            return res.status(200).send(item);
        } catch (error: any) {
