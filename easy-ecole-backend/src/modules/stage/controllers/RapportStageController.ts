...
import { RapportStage } from "../models/RapportStage";

export default class RapportStageController {
    constructor() { }

    static async getAll(req: Request, res: Response): Promise<Response> {
        try { const items = await RapportStage.findAll(); return res.status(200).send(items); }
        catch (error) { return res.status(500).json({ success: false, error: error }); }
    }

    static async get(req: Request, res: Response): Promise<Response> {
        try {
            const item = await RapportStage.findByPk(req.params.id);
    ...
    static async create(req: Request, res: Response): Promise<Response | null> {
        ...
        try {
            const item = await RapportStage.create({ ...req.body });
            return res.status(200).send(item);
        } catch (error: any) {
