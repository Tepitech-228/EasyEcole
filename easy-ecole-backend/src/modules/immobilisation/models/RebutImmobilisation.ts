import { Model, InferAttributes, InferCreationAttributes, CreationOptional, DataTypes, ForeignKey } from "sequelize";
import { DatabaseConnection } from "../../../core/helpers/DatabaseConnection";
import { MODULE_MODEL_PREFIX, MODULE_TABLE_PREFIX } from "../ImmobilisationModule";
import { Immobilisation } from "./Immobilisation";

export class RebutImmobilisation extends Model<InferAttributes<RebutImmobilisation>, InferCreationAttributes<RebutImmobilisation>> {
  declare id: CreationOptional<string>
  declare immobilisationId: ForeignKey<Immobilisation['id']>
  declare dateRebut: CreationOptional<Date>
  declare motif: string
  declare montant: CreationOptional<number>
  declare approuvePar: CreationOptional<string>
  declare dateApprobation: CreationOptional<Date>
  declare readonly createdAt: CreationOptional<Date>
  declare readonly updatedAt: CreationOptional<Date>
}

RebutImmobilisation.init({
  id: { type: DataTypes.INTEGER.UNSIGNED, autoIncrement: true, primaryKey: true },
  immobilisationId: { type: DataTypes.INTEGER.UNSIGNED, allowNull: false },
  dateRebut: { type: DataTypes.DATEONLY, allowNull: true },
  motif: { type: DataTypes.TEXT, allowNull: false },
  montant: { type: DataTypes.DECIMAL(12, 2), allowNull: true },
  approuvePar: { type: DataTypes.STRING, allowNull: true },
  dateApprobation: { type: DataTypes.DATE, allowNull: true },
  createdAt: DataTypes.DATE,
  updatedAt: DataTypes.DATE,
}, {
  sequelize: DatabaseConnection.getInstance().sequelize,
  paranoid: true,
  modelName: MODULE_MODEL_PREFIX + 'RebutImmobilisation',
  tableName: MODULE_TABLE_PREFIX + 'rebuts_immobilisation',
  timestamps: true
})
