...
export default class LocalisationController {
    static async getAll(req: Request, res: Response): Promise<Response> {
        try { const items = await Localisation.findAll(); return res.status(200).send(items); }
    ...
    static async create(req: Request, res: Response): Promise<Response | null> {
        ...
        try {
            const item = await Localisation.create({ ...req.body });
            return res.status(200).send(item);
        } catch (error: any) {
