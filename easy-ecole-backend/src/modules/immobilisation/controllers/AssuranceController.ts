...
export default class AssuranceController {
    static async getAll(req: Request, res: Response): Promise<Response> {
        try { const items = await Assurance.findAll(); return res.status(200).send(items); }
        catch (error) { return res.status(500).json({ success: false, error: error }); }
    }
    static async get(req: Request, res: Response): Promise<Response> {
        try {
            const item = await Assurance.findByPk(req.params.id);
    ...
    static async getByImmobilisation(req: Request, res: Response): Promise<Response> {
        try {
            const items = await Assurance.findAll({ where: { immobilisationId: req.params.immobilisationId } });
    ...
    static async create(req: Request, res: Response): Promise<Response | null> {
        ...
        try {
            const item = await Assurance.create({ ...req.body });
            return res.status(200).send(item);
        } catch (error: any) {
