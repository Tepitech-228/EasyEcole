...
export default class LigneInventaireController {
    static async getAll(req: Request, res: Response): Promise<Response> {
        try { const items = await LigneInventaire.findAll(); return res.status(200).send(items); }
        catch (error) { return res.status(500).json({ success: false, error: error }); }
    }
    static async get(req: Request, res: Response): Promise<Response> {
        try {
            const item = await LigneInventaire.findByPk(req.params.id);
    ...
    static async create(req: Request, res: Response): Promise<Response | null> {
        ...
        try {
            const item = await LigneInventaire.create({ ...req.body });
            return res.status(200).send(item);
        } catch (error: any) {
    ...
    static async getByInventaire(req: Request, res: Response): Promise<Response> {
        try {
            const items = await LigneInventaire.findAll({
