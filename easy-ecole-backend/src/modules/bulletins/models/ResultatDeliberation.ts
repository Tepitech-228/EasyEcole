...
export class ResultatDeliberation extends Model<InferAttributes<ResultatDeliberation>, InferCreationAttributes<ResultatDeliberation>> {
  ...
  declare assiduite: CreationOptional<string>
  declare situationFinanciere: CreationOptional<string>
...
ResultatDeliberation.init({
  ...
  decision: { type: DataTypes.ENUM('admis', 'rattrapage', 'redouble', 'admis_avec_dette', 'ajourne', 'exclu', 'derogation'), defaultValue: 'admis', allowNull: false },
  assiduite: { type: DataTypes.ENUM('assidu', 'moyen', 'insuffisant'), allowNull: true },
  situationFinanciere: { type: DataTypes.ENUM('reglee', 'impayee', 'partielle'), allowNull: true },
