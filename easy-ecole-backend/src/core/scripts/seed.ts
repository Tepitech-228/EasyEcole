...
async function seed() {
    ...
    const sess1 = await InsSess.create({ libelle: 'Session 2024-2025', dateDebut: new Date('2024-10-01'), dateFin: new Date('2025-07-31'), anneeAcademiqueId: an1.id, niveauEtudeId: nivIns1.id, });
    console.log('  ✓ Session');
    ...
    // ── Matières prérequis (Inscription) ──
    const insMat1 = await InsMat.create({ libelle: 'Mathématiques niveau Bac' });
    const insMat2 = await InsMat.create({ libelle: 'Français niveau Bac' });
    const insMat3 = await InsMat.create({ libelle: 'Anglais niveau Bac' });
    const insMat4 = await InsMat.create({ libelle: 'Informatique niveau Bac' });
    console.log('  ✓ Matières prérequis');

    for (const p of [parcIns1, parcIns2, parcIns3, parcIns4]) {
        await InsPreP.create({ parcoursId: p.id, niveauEtudeId: nivIns1.id, matierePrerequisId: insMat3.id, noteRequise: 10, typeEvaluation: 'moyenne', periodeEvaluation: 'semestre_1' });
    }
    console.log('  ✓ Prérequis parcours');

    // ── Séances de cours ──
    const allCoursList = [cours1, cours2, cours3, cours4, cours5, cours6, cours7, cours8];
    const jours = ['1', '2', '3', '4', '5'];
    const salles = ['Amphi A', 'Salle 101', 'Labo Info'];
    for (let ci = 0; ci < allCoursList.length; ci++) {
        const c = allCoursList[ci];
        await InsSeance.create({ titre: `Séance ${c.code}`, jourSemaine: jours[ci % jours.length], salle: salles[ci % salles.length], dateDebut: new Date('2024-10-01'), dateFin: new Date('2025-07-31'), heureDebut: '08:00', heureFin: '10:00', description: `Séance hebdomadaire de ${c.intitule}`, coursId: c.id, enseignantId: c.enseignantId, salleDeClasseId: salle1.id });
    }
    console.log('  ✓ Séances de cours');

    // ── Cahiers de texte ──
    for (const c of allCoursList) {
        const cahier = await InsCahier.create({ titre: `Cahier - ${c.code}`, description: `Suivi pédagogique de ${c.intitule}`, coursId: c.id, enseignantId: c.enseignantId });
        await InsBlocCah.create({ cahierDeTexteId: cahier.id, date: new Date('2025-02-10'), heureDebut: '08:00', heureFin: '10:00', contenu: `Séance 1 : Introduction au cours ${c.intitule}. Présentation du syllabus et des objectifs pédagogiques.` });
        await InsBlocCah.create({ cahierDeTexteId: cahier.id, date: new Date('2025-02-17'), heureDebut: '08:00', heureFin: '10:00', contenu: `Séance 2 : Suite du programme. Exercices pratiques et études de cas.` });
    }
    ...
    // ── Présences ──
    const presenceCoursList = allCoursList.slice(0, 3);
    for (const c of presenceCoursList) {
        const liste = await InsLPres.create({ titre: `Présence - ${c.code}`, description: `Feuille de présence pour ${c.intitule}`, coursId: c.id, enseignantId: c.enseignantId });
        const presence = await InsPres.create({ listePresenceId: liste.id, date: new Date('2025-02-10'), heureDebut: '08:00', heureFin: '10:00' });
        const coursParts = await InsCourP.findAll({ where: { coursId: c.id } });
        for (const cp of coursParts) {
            await InsPresCP.create({ presenceId: presence.id, coursParticipantId: cp.id, etatDePresence: Math.random() > 0.2 ? 'present' : 'absent' });
        }
    }
    console.log('  ✓ Présences');

    // ── Audit notes ──
    const someNotes = await InsNoteEval.findAll({ limit: 5 });
    for (const n of someNotes) {
        const oldNote = +(n.note - 1.5).toFixed(2);
        n.note = +(n.note + 0.5).toFixed(2);
        await n.save();
        await AuditNote.create({ noteEvaluationId: n.id, ancienneNote: oldNote, nouvelleNote: n.note, modifiePar: admin.id, motif: 'Correction après réclamation' });
    }
    ...
    // ── Conventions et rapports de stage ──
    const allStageDems = await StgDem.findAll();
    for (const dem of allStageDems) {
        await StgConv.create({ demandeStageId: dem.id, fichier: `convention_${dem.id}.pdf`, dateSignature: new Date('2025-06-15') });
        await StgRap.create({ demandeStageId: dem.id, fichier: `rapport_${dem.id}.pdf`, dateSoumission: new Date('2025-08-31') });
    }
    console.log('  ✓ Conventions et rapports de stage');
    ...
    await ScolEven.create({ titre: 'Début des cours', date: new Date('2024-10-01'), description: 'Rentrée académique 2024-2025', type: 'cours' });
